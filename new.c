...
write_com(0x06); //增量方式不移位 显竟獗暌贫 柚? write_com(0x80); //检测忙信号
write_com(0x01); //显示开，光标关，不闪烁 num_1=0;
i=0;
j=0;
a=0; //第一个参与运算的数
b=0; //第二个参与运算的数
...
else if(fuhao==2){write_com(0x80+0x4f);//光标前进至第二行最后一个显示处 write_com(0x04); //设置从后住前写数据，每写完一个数据，光标后退一格(这个照理说顺序不对，可显示和上段一样)
if(a-b>0)
c=a-b;
else
c=b-a;
...
else if(fuhao==3){write_com(0x80+0x4f); write_com(0x04);
c=a*b;
while(c!=0)
{
write_date(0x30+c%10);
